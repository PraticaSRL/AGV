...
int main(int argc, char* argv[])
{
	...
	//Matrice covarianza
  	double pcov_ok[36] = { 1e-5,    0,   0,   0,  0, 0,
                                  0, 1e-5,   0,   0,  0, 0,
                                  0,   0, 1e6,   0,   0, 0,
                                  0,   0,   0, 1e6,   0, 0,
                                  0,   0,   0,   0, 1e6, 0,
	...
	while (ros::ok() && run_forever)
  	{
		...
		//if(buf[0]==0x52 && buf[1]==0x01)
		//{
		//	ROS_INFO("A=%d B=%d C=%d", buf[11], buf[12], buf[14]);
		//}		
		
		if(buf[0]==0x52 && buf[1]==0x01 && buf[11] == 0x67 && buf[12]==0x00 && buf[14]==0x00)
		{
			//Conversioni X e Y
			memcpy(&i32, &buf[24],4);
      ...
      contatore=0;
		  ...
		  //Invia solo se x>-3.5m (altrimenti èoutdoor-->dato non attendibile)
      if(x>=-3.5){
